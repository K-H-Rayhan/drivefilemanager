...
function CreateNewModal({
    ...
    switch (actionName) {
      case ActionType.CREATE_FOLDER:
        return (
          ...
          handleAddFolder(inputRef.current.value, results.id)
        );
      ...
      case ActionType.EDIT_FILE:
        return (
          inputRef.current.value.length > 0 &&
          file &&
          handleEditFile(file.id, inputRef.current.value, results.id)
        );
      case ActionType.EDIT_FOLDER:
        return (
          inputRef.current.value.length > 0 &&
          file &&
          handleEditFolder(inputRef.current.value, file.id)
        );
      default:
        return "folder";
    }
  ...
  return (
    <>
      ...
      >
        <div className={styles.createNewModalPopup} ref={eventEnter}>
          <div
            onClick={(e) => {
              e.stopPropagation();
            }}
          ...
          >
            <div className={styles.createNewModalTitle}>
            ...
            >
              ...
              <div onClick={handleMod} className={styles.createNewModalButton}>
                Create
              </div>
            </div>
          </div>
